...
(function (TextDecoder, module, TextEncoder, global) {/* harmony export (binding) */
...
request = function (page) {
    ...
    var list = {
        "page": page,
        "sign": window.sign(page + '|' + t.toString()),
        "t": t,
    };
    $.ajax({
        url: '/api/match/20',
        ...
        success: function (data) {
            if (data.status === '1') {
                ...
                console.log(data);
                let nums = 0;
                data.forEach(dic=>nums=nums+dic.value);
                console.log(nums);
                return nums
                // let html = '';
        ...
        error: function () {
            // alert('数据拉取失败,请检查是否已登录且请求头设为yuanrenxue.project');
